...
router.get('/', (req, res) => {
  ...
  db.Select('xx_work', con, (err, response) => {
    res.send(response);
  });
});
...
router.post('/', (req, res) => {
  ...
  db.Insert('xx_work', insertData, (err, response) => {
    res.send(response);
  });
});
...
router.put('/:id', (req, res) => {
  ...
  db.Update('xx_work', updateData, con, (err, response) => {
    res.send(response);
  });
});
...
router.delete('/:id', (req, res) => {
  const con = [];
  con.id = parseInt(req.params.id, 10);
  db.Delete('xx_work', con, (err, response) => {
    res.send(response);
  });
});
